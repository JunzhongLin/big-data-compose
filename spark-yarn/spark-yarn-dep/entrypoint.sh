...
if [ -z "${SPARK_MASTER_ADDRESS}" ]; then
  ...
  "${SPARK_HOME}/sbin/start-master.sh" -h master >> $SPARK_MASTER_LOG
  "${SPARK_HOME}/sbin/start-history-server.sh" &
  # cd /usr/spark/bin && ./spark-class org.apache.spark.deploy.master.Master \
  ...
  "${SPARK_HOME}/sbin/start-slave.sh" "${SPARK_MASTER_ADDRESS}" >> $SPARK_WORKER_LOG
  # cd /usr/spark/bin && ./spark-class org.apache.spark.deploy.worker.Worker \
  # --webui-port $SPARK_WORKER_WEBUI_PORT $SPARK_MASTER_ADDRESS >> $SPARK_WORKER_LOG
